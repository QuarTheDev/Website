...
function activateEasterEgg() {
  ...
  const change1 = document.querySelector('#name');
  const change2 = document.querySelector('#profession');
  change1.textContent = 'let\'s go';
  change2.textContent = 'it\'s /dev/urandom time';
  ...
  setTimeout(function() {
    ...
    function changeRandomColor() {
      const elements = document.querySelectorAll('html *:not(script)');
    ...
    function getRandomDisplacement() {
      const displacement = Math.floor(Math.random() * 100) - 5;
    ...
    setInterval(appendRandomCharacter, 250);
